...
\begin{document}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{0.45\textwidth}
			\fbox{\includegraphics[width=\linewidth]{figure/ex_2_5_fig_1a}}
			\caption{$q_{*}(a)$ is stationary}
		\end{subfigure}
		\begin{subfigure}{0.45\textwidth}
			\fbox{\includegraphics[width=\linewidth]{figure/ex_2_5_fig_1b}}
			\caption{$q_{*}(a)$ is stationary}
			\label{f1}
		\end{subfigure}
		\begin{subfigure}{0.45\textwidth}
			\fbox{\includegraphics[width=\linewidth]{figure/ex_2_5_fig_2a}}
			\caption{$\mathbb{E}\left[ q_{*}(a) \right]$ increases by 0.01 on every timestep}
		\end{subfigure}
		\begin{subfigure}{0.45\textwidth}
			\fbox{\includegraphics[width=\linewidth]{figure/ex_2_5_fig_2b}}
			\caption{$\mathbb{E}\left[ q_{*}(a) \right]$ increases by 0.01 on every timestep}
			\label{f2}
		\end{subfigure}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\fbox{\includegraphics[width=0.5\linewidth]{figure/ex_2_10_fig_1}}
	...
	\begin{figure}[H]
		\centering
		\fbox{\includegraphics[width=0.5\linewidth]{figure/ex_3_20_fig_1}}
	...
	\begin{figure}[H]
		\centering
		\fbox{\includegraphics[width=0.5\linewidth]{figure/ex_3_21_fig_1}}
	...
	\noindent
 	...
 	\begin{algorithm}[H]
 		...
 		\algnewcommand{\Goto}[1]{\algorithmicgoto~\hyperref[#1]{\textsc{Policy Evaluation}}}%
 		\caption{Policy Iteration for $q_*(s,a)$}\label{policy-iter-q	}
 		\begin{algorithmic}[1]
 			\Require Initialize $Q(s,a)$ and $\pi(s)\in\mathcal{A}(s)$ for all $a\in\mathcal{A}(s)$ and $s\in\mathcal{S}$ arbitrarily. 
 			\Ensure $Q \approx q_*$ and $\pi \approx \pi_*$.
 			\Procedure{Policy Evaluation}{}\Comment{Compute $v_\pi$ from $\pi$}\label{policy-eval}
 			\Do\State $\Delta \leftarrow 0$
 				\For{$s\in\mathcal{S}$}
 					\For{$a\in\mathcal{A}(s)$}
 						\State $q \leftarrow Q(s,a)$
 						\State $Q(s,a) \leftarrow \sum_{s',r} p(s',r|s,a) \left[ r+\gamma Q(s,\pi(s)) \right]$
 						\State $\Delta \leftarrow \max\{ \Delta, |q-Q(s,a) | \}$
 					\EndFor
 				\EndFor
 			\doWhile{$\Delta\geq\theta$}
 			\EndProcedure
 			\State
 			\Procedure{Policy Improvement}{}\Comment{Get improved policy $\pi$ using state-action values $q_\pi$}
 			\State $\textit{policy-stable}\leftarrow\textit{true}$
 			\For{$s\in\mathcal{S}$}
 				\State $\textit{old-action} \leftarrow\pi(s)$
 				\State $\pi(s) = \arg\max\limits_{a\in\actionspa} \sum_{s\in\statesp}\sum_{r\in\rewardsp} p(s',r|s,a) \left[ r + \gamma Q(s,\pi(s)) \right]$
 				\State If $Q(s,\textit{old-action}) \neq Q(s,\pi(s))$ then $\textit{policy-stable}=\textit{false}$
 			\EndFor
 			\If{$\textit{policy-stable}$}
 				\Return $Q$ and $\pi$
 			\Else
 				\State \Goto{policy-eval}
			\EndIf			
 			\EndProcedure
 		\end{algorithmic}
 	\end{algorithm}
 	...
 	\begin{figure}[H]
 		\centering
 		\begin{subfigure}{0.45\textwidth}
 			\fbox{\includegraphics[width=\linewidth]{figure/ex_4_9_fig_1.pdf}}
 			\caption{Optimal state value for $p_h=0.25$}
 		\end{subfigure}
 		\begin{subfigure}{0.45\textwidth}
 			\fbox{\includegraphics[width=\linewidth]{figure/ex_4_9_fig_2.pdf}}
 			\caption{Optimal actions for $p_h=0.25$}
 		\end{subfigure}
 		\begin{subfigure}{0.45\textwidth}
 			\fbox{\includegraphics[width=\linewidth]{figure/ex_4_9_fig_3.pdf}}
 			\caption{Optimal state value for $p_h=0.55$}
 		\end{subfigure}
 		\begin{subfigure}{0.45\textwidth}
 			\fbox{\includegraphics[width=\linewidth]{figure/ex_4_9_fig_4.pdf}}
 	...
 	The following results were derived by implementing Monte Carlo (MC) method for calculating value function in case of the blackjack problem (as given in the textbook).
 	 \begin{figure}[H]
 	 	\centering
 		\begin{subfigure}{0.45\textwidth}
 			\fbox{\includegraphics[width=\linewidth]{figure/ch_5_extra_fig_1.pdf}}
 		\end{subfigure}
 		\begin{subfigure}{0.45\textwidth}
 			\fbox{\includegraphics[width=\linewidth]{figure/ch_5_extra_fig_2.pdf}}
 		\end{subfigure}
 		\begin{subfigure}{0.45\textwidth}
 			\fbox{\includegraphics[width=\linewidth]{figure/ch_5_extra_fig_3.pdf}}
 		\end{subfigure}
 		\begin{subfigure}{0.45\textwidth}
 			\fbox{\includegraphics[width=\linewidth]{figure/ch_5_extra_fig_4.pdf}}
 		\end{subfigure}
 	...
 	\begin{figure}[H]
 		\centering
 		\fbox{\includegraphics[width=0.1\linewidth]{figure/ex_5_3_fig_1}}
